...
class stemmer {
	...
	public static void main(String[] args) throws IOException {
		...
		// path3 = D:\TDT\DOC\Acts of Violence or
		// War\20030401_0113_1041_AFP_ARB\AFA20030401.0113.0007\nonNE.txt
		String read_dir = "C:\\Users\\wangsy\\Desktop\\Code\\TDT\\Result\\path\\path5.txt";
		ArrayList<File> files = getListFiles(read_dir);
		for (File file : files)
			try {
				FileInputStream in = new FileInputStream(file);
				System.out.println(file.toString());
				File f = make_file(file.toString());
	...
	public static File make_file(String path) throws IOException {
		String writepath = path.replace(".txt", "");
		File f = new File(writepath + "afterStem.txt");
		if (!f.exists()) {
			f.createNewFile();
		}
		return f;
	...
	public static ArrayList<File> getListFiles(String path) {
		...
		try {
			if (file.isFile() && file.exists()) {
				...
				// 读取每一个文件所在的目录
				while ((readpath = bufferedReader.readLine()) != null) {
					File doc = new File(readpath);
					files.add(doc);
				}
			}
		} catch (Exception e) {
